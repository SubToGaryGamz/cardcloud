...
- ✅ **Public Leaderboard at `/leaderboard`** — no-auth, opt-out by default-off (everyone in until they hide). Profit metric uses 0..1 `bar` (relative to leader, hides raw $ for privacy) + tier pills (Diamond/Platinum/Gold/Silver/Bronze). `cards` metric shows total card count. Top-3 podium + ordered list. Annual subscribers get a gold "PRO MEMBER" badge; monthly Pros get red "PRO" pill.
- ✅ **Referral system** — `?ref=CODE` on Landing stores `cv_referral` in localStorage; `Login.jsx` register form auto-sends `referral_code` to `POST /api/auth/register`. Backend `referred_by` is recorded on signup; `referred_count` increments on signup, and `rewards_given_months` (+30 days for both referrer & referee) increments on the referee's first paid Stripe checkout. Profile shows the user's link, copy button, "friends joined" + "rewards earned" counters.
- ✅ **Leaderboard preferences** in Profile — "Hide me" (opt-out), "Show my real name" (replaces anon handle), "Custom handle" (24 chars, alphanum + space/_/-). Persists via `PUT /api/me/leaderboard-prefs`.
- ✅ **Monthly Profit Goal tile** on Dashboard — `MonthlyGoalTile` (next to Best Flip card) shows current-month profit + flips count. Edit goal inline → progress bar (green→yellow gradient when ≥100%). Persists via `PUT /api/me/goal` (null clears).
- ✅ **Year-in-Review modal** — "My Recap" button in Dashboard header opens a 4:5 IG-Stories-friendly card showing total profit, flips, cards added, spend, MVP flip, top sport, with red+gold gradient + CardCloud branding. "Download PNG" button uses `html-to-image` (lazy import) to export a 2x pixel-ratio shareable image. January auto-prompt fires once per user per year.
- ✅ **Annual Pro badge** — `is_annual_pro` flag added to `/api/billing/me` and the leaderboard rows. Yearly subscribers get a gold "PRO MEMBER" pill across the leaderboard (rank #1 podium + every list row).
- ✅ **Robust clipboard helper** (`lib/clipboard.js`) — modern `navigator.clipboard.writeText` with legacy `document.execCommand("copy")` fallback for headless / iframe / restricted-context browsers. Used by Profile (referral, vault), CollectionCard (per-card share), CsvImportHelp (sample CSV).
- ✅ **Bulk delete cards** (Feb 2026) — Dashboard "Select" button toggles select mode: each tile becomes a click-to-toggle checkbox, lightbox/swipe disabled, selected tiles get a red ring. Floating action bar shows live count + "Select all/Clear" + red "Delete" CTA. Confirms before deleting, then `POST /api/cards/bulk-delete {card_ids:[...]}` removes them in a single owner-scoped Mongo `delete_many`, soft-deletes attached image files, and returns `{deleted: N}`. Tested 9/9 backend pytest + 100% frontend e2e.
- ✅ **HEIC/HEIF auto-conversion** (Feb 2026) — Client-side `lib/heicConvert.js` lazy-loads `heic2any` (only when needed) to transparently convert iPhone HEIC photos to JPEG before upload. Wired into card images, AI photo intake, and avatar uploads. File pickers updated to surface `.heic/.heif`. Backend unchanged.
- ✅ **Admin Analytics** (Feb 2026) — New `/admin` route gated by `ADMIN_EMAILS` env var (currently `bmangrill@gmail.com,demo@example.com`, comma-separated, case-insensitive). Surfaces operator-level metrics: total/30d signups + 30-day signup line chart, active Pro count (monthly vs annual), est. MRR, 30d & lifetime Stripe revenue, total cards/sold/profit logged across the platform, AI scan total + 30-day, top sports bar chart, beta redemptions, referral signups + top 5 referrers. Backend tracks AI-scan events in `db.ai_scan_events`; `is_admin` flag hydrated via shared `_hydrate_user()` helper across login/register/session/me. Tested 15/15 backend pytest + frontend e2e (one HIGH bug found mid-test — login response wasn't hydrating `is_admin` so admin nav was hidden until reload — fixed by routing all auth handlers through `_hydrate_user()`).
